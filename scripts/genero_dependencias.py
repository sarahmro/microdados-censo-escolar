...
for ano in anos:
    print(f"Processando gênero por dependência administrativa - {ano}")
    ...
    df_final = df.groupby(
        ["SG_UF", "NO_UF", "NO_REGIAO", "TP_DEPENDENCIA"],
        as_index=False
    ).agg(
    ...
    df_final = df_final[[
        "ANO", "NO_REGIAO", "SG_UF", "NO_UF", "TP_DEPENDENCIA",
        "QT_MAT_BAS_FEM", "QT_MAT_BAS_MASC", "QT_MAT_BAS_TOTAL",
        "PERC_FEM",
        "QT_MAT_FUND", "QT_MAT_FUND_AI", "QT_MAT_FUND_AF", "QT_MAT_MED"
    ]].sort_values(by=["NO_REGIAO", "SG_UF", "TP_DEPENDENCIA"])

    df_final.to_csv(
        f"{pasta_saida}/genero_dependencia_{ano}.csv",
        sep=";", index=False, encoding="utf-8-sig"
    )

    print(f"{ano} concluído. {len(df_final)} registros.")

print("Arquivos de gênero por dependência gerados.")